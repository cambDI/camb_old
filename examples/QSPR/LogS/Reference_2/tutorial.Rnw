...
<<highlight=TRUE,tidy.opts=list(width.cutoff=50),tidy=TRUE,warning=FALSE,eval=FALSE>>=
...
predictions <- PredictExternal(test_structures_file, std.options, descriptor.types, dataset, readRDS("rf.rds"))
print(predictions)
@
