...
The function {\it StandardiseMolecules} enables the depiction of molecular structures in the same (standardized) way.
The different arguments of this function allow to control the maximum number of (i) fluorines, (ii) chlorines,
(iii) bromines, and (iv) iodines the molecules can exhibit in order to be kept. If using the default value, {\it i.e.} "-1",
all molecules will be kept irrespective of the exhibited number of fluorines, chlorines, bromines, and iodines.
...
are written to the file indicated in the argument "output". 
In this case the file is \verb|"standardisation_COX_info.csv"|.
Molecules that pass the filters are written to the file indicated in the argument "standardised.file".
By contrast, molecules that do not pass these filters are written to the file indicated in the "removed.file" argument.
...
{\centering \includegraphics[width=12cm]{figure/unnamed-chunk-21} 

}

\caption[Example of compoud depiction]{Example of compoud depiction.\label{fig:unnamed-chunk-21}}
\end{figure}
...
\caption[Maximum achievable model performance]{Maximum achievable model performance\label{fig:unnamed-chunk-27}}
\end{figure}
