...
(i) binary format: \verb|{\it output}_unhashed_binary_EXT.csv|, and (ii) counts format: 
\verb|{\it output}_unhashed_counts_EXT.csv|.
...
\verb|hashed_binaryEXT|, \verb|hashed_countsEXT|, \verb|unhashed_binaryEXT| and \verb|unhashed_countsEXT|.
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{amino_acid_IDs_zscales} \hlkwb{<-} \hlkwd{AA_descs}\hlstd{(}\hlkwc{Data} \hlstd{= amino_acid_IDs,}
...
\item PCs_All : a dataframe containing all PCs.
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{target_PCA} \hlkwb{<-} \hlkwd{PCA}\hlstd{(}\hlkwc{Data} \hlstd{= amino_acid_IDs_zscales,} \hlkwc{RowNames} \hlstd{= dataset}\hlopt{$}\hlstd{accession,}
    \hlkwc{cor} \hlstd{=} \hlnum{TRUE}\hlstd{,} \hlkwc{scale} \hlstd{=} \hlnum{TRUE}\hlstd{,} \hlkwc{center} \hlstd{=} \hlnum{TRUE}\hlstd{)}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error: could not find function "{}PCA"{}}}\begin{alltt}
\hlstd{plot_PCA_COX} \hlkwb{<-} \hlkwd{PCAPlot}\hlstd{(target_PCA}\hlopt{$}\hlstd{Data,} \hlkwc{PointSize} \hlstd{=} \hlnum{10}\hlstd{,}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error: could not find function "{}PCAPlot"{}}}\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{max_performance} \hlkwb{<-} \hlkwd{MaxPerf}\hlstd{(}\hlkwc{meanNoise} \hlstd{=} \hlnum{0}\hlstd{,} \hlkwc{sdNoise} \hlstd{=} \hlnum{0.6}\hlstd{,} \hlkwc{meanResp} \hlstd{=} \hlkwd{mean}\hlstd{(bioactivity),}
    \hlkwc{sdResp} \hlstd{=} \hlkwd{sd}\hlstd{(bioactivity),} \hlkwc{lenPred} \hlstd{=} \hlnum{800}\hlstd{,} \hlkwc{tmar} \hlstd{=} \hlnum{0.2}\hlstd{,} \hlkwc{bmar} \hlstd{=} \hlnum{0.2}\hlstd{,}
    \hlkwc{rmar} \hlstd{=} \hlnum{0.2}\hlstd{,} \hlkwc{lmar} \hlstd{=} \hlnum{0.2}\hlstd{)}

\hlkwd{grid.arrange}\hlstd{(max_performance}\hlopt{$}\hlstd{p1, max_performance}\hlopt{$}\hlstd{p2, max_performance}\hlopt{$}\hlstd{p3,}
    \hlstd{max_performance}\hlopt{$}\hlstd{p4,} \hlkwc{nrow} \hlstd{=} \hlnum{2}\hlstd{)}
\end{alltt}
\end{kframe}\begin{figure}[]
...
The dataset without repeated repetitions will be saved to the file "Whole_dataset.rds".
...
{\ttfamily\noindent\color{warningcolor}{\#\# Warning: cannot open compressed file 'Whole\_dataset.rds', probable reason 'No such file or directory'}}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error: object 'Name' not found}}\end{kframe}
\end{knitrout}
