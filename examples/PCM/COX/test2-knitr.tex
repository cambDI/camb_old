...
{\ttfamily\noindent\color{warningcolor}{\#\# Warning: package 'png' was built under R version 3.0.2}}\end{kframe}
\end{knitrout}
...
\hlkwd{saveRDS}\hlstd{(descriptors,} \hlkwc{file} \hlstd{=} \hlstr{"Padel_COX.rds"}\hlstd{)}
\hlstd{descriptors} \hlkwb{<-} \hlkwd{readRDS}\hlstd{(}\hlstr{"Padel_COX.rds"}\hlstd{)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error: object 'amino\_compound\_IDs\_zscales' not found}}\begin{alltt}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error: object 'target\_PCA' not found}}\begin{alltt}
\hlkwd{pdf}\hlstd{(}\hlstr{"PCA_seqs.pdf"}\hlstd{)}
\hlstd{plot_PCA_Cox}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error: object 'plot\_PCA\_Cox' not found}}\begin{alltt}
\hlkwd{dev.off}\hlstd{()}
\end{alltt}
\begin{verbatim}
## pdf 
...
\hlkwd{pdf}\hlstd{(}\hlstr{"Pairwise_distance.pdf"}\hlstd{)}
\hlstd{plot_pwd}
\hlkwd{dev.off}\hlstd{()}
\end{alltt}
\begin{verbatim}
...
{\ttfamily\noindent\color{warningcolor}{\#\# Warning: package 'doMC' was built under R version 3.0.2}}

{\ttfamily\noindent\itshape\color{messagecolor}{\#\# Loading required package: foreach\\\#\# Loading required package: iterators}}\begin{alltt}
...
{\ttfamily\noindent\color{warningcolor}{\#\# Warning: cannot open compressed file 'COXrf.rds', probable reason 'No such file or directory'}}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error: object 'modelCoxRF' not found}}\begin{alltt}
...
## function (model, digits = 3) 
## {
##     model$results$Rsquared[which(model$results$RMSE %in% min(model$results$RMSE, 
##         na.rm = TRUE))]
## }
## <environment: namespace:camb>
\end{verbatim}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error: error in evaluating the argument 'object' in selecting a method for function 'predict': Error: object 'modelCoxRF' not found}}\end{kframe}
\end{knitrout}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error: object 'holdout.predictions' not found}}\begin{alltt}
\hlstd{MetricsRf}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error: object 'MetricsRf' not found}}\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{pdf}\hlstd{(}\hlstr{"ObsPred.pdf"}\hlstd{)}
